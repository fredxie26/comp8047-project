...
if __name__ == '__main__':
    with open(ca_file_name, 'a', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
        if os.stat(ca_file_name).st_size == 0:
            writer.writerow(
                ['job_cat', 'loc', 'job_type', 'job_count', 'prev_range', 'search_date'])
    ca_params = []

    for location in canada_states:
        for job in jobs:
            ca_params.append([location, job, look_back_range, 'ca'])
    
    pool = Pool(processes=5)
    pool.map(load_results, ca_params)
    pool.close()
    pool.join()
